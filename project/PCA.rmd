...
for(i in 1:(nrow(application$first_sample) / 4)) {
  application$first_sample_pred <-
    window(application$first_sample,
           end = application$first_sample_period$startdate[1] + i)
  
  application$CPIAUCSL_model$x <-
    model.matrix(CPIAUCSL ~ ., data = application$first_sample_pred)[, -1]
  application$CPIAUCSL_model$y <-
    application$first_sample_pred[, "CPIAUCSL"]
  
  application$FYFF_model$x <-
    model.matrix(FYFF ~ ., data = application$first_sample_pred)[, -1]
  application$FYFF_model$y <-
    application$first_sample_pred[, "FYFF"]
  
  application$IPS10_model$x <-
    model.matrix(IPS10 ~ ., data = application$first_sample_pred)[, -1]
  application$IPS10_model$y <-
    application$first_sample_pred[, "IPS10"]
  
  application$LHUR_model$x <-
    model.matrix(LHUR ~ ., data = application$first_sample_pred)[, -1]
  application$LHUR_model$y <-
    application$first_sample_pred[, "LHUR"]
  ...
  for (j in PCR_ncomp) {
    ...
    pcr$CPIAUCSL_model$MSE[i, as.character(j)] <-
      get_MSE(pcr$CPIAUCSL_model$predicted,
              application$CPIAUCSL_model$y)
  ...
  for (g in PLSR_ncomp) {
    ...
    plsr$CPIAUCSL_model$MSE[i, as.character(g)] <-
      get_MSE(plsr$CPIAUCSL_model$predicted,
              application$CPIAUCSL_model$y)
