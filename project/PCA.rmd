...
plotting <- new.env()
plotting$simulation_option_default <- list(N = possible_N[1],
                                  p = 20,
                                  runs = 100)
plotting$dgp_option_default <- list(BETAS_INCREASE_FACTOR = 4)
plotting$sim_option <- plotting$simulation_option_default
plotting$sim_option$runs <- 1
plotting$sim_option$N <- 100
plotting$sim_option$p <- 10
plotting$result <- simulation(dgp_option_list = plotting$dgp_option_default, simulation_option_list = plotting$sim_option)

# Show 3 plots on a row in 2 rows
# mar argument makes the plot title visible(corrplot bug)
{par(mfrow=c(2,3))
corrplot::corrplot(cor(plotting$result$data[[1]]), title = "Case I", mar=c(0,0,2,0))
corrplot::corrplot(cor(plotting$result$data[[2]]), title = "Case II", mar=c(0,0,2,0))
corrplot::corrplot(cor(plotting$result$data[[3]]), title = "Case III", mar=c(0,0,2,0))
corrplot::corrplot(cor(plotting$result$data[[4]]), title = "Case IV", mar=c(0,0,2,0))
corrplot::corrplot(cor(plotting$result$data[[5]]), title = "Case V", mar=c(0,0,2,0))}
```
